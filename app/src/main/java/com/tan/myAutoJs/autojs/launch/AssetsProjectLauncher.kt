package com.tan.myAutoJs.autojs.launch
...
import com.tan.myAutoJs.App
import com.example.myautojs.BuildConfig

import com.tan.myAutoJs.autojs.Pref
...
//            }
    ...
    private fun runScript(activity: Activity?) {
        if (mScriptExecution != null && mScriptExecution!!.engine != null && !mScriptExecution!!.engine.isDestroyed) {
            return
        }
        try {
            ...
            mScriptExecution = App.autoJs.scriptEngineService.execute(source, config)
        } catch (e: Exception) {
            App.autoJs.globalConsole.error(e)
        }
